...
using CloudFabric.Projections.OpenSearch.Helpers;
...
using Polly;
using SortOrder = OpenSearch.Client.SortOrder;

namespace CloudFabric.Projections.OpenSearch;

public class OpenSearchProjectionRepository<TProjectionDocument> : OpenSearchProjectionRepository, IProjectionRepository<TProjectionDocument>
    where TProjectionDocument : ProjectionDocument
{
    public OpenSearchProjectionRepository(
...
public class OpenSearchProjectionRepository : ProjectionRepository
{
    private readonly ProjectionDocumentSchema _projectionDocumentSchema;
    private string? _keyPropertyName;
    private readonly OpenSearchClient _client;
    private readonly OpenSearchProjectionIndexer _indexer;
    private readonly ILogger<OpenSearchProjectionRepository> _logger;
    ...
    ) : base(projectionDocumentSchema, loggerFactory.CreateLogger<ProjectionRepository>())
    {
        ...
        _logger = loggerFactory.CreateLogger<OpenSearchProjectionRepository>();
        ...
        _indexer = new OpenSearchProjectionIndexer(awsAuthConnectionSettings, loggerFactory);
    ...
    ) : base(projectionDocumentSchema, loggerFactory.CreateLogger<ProjectionRepository>())
    {
        ...
        _indexer = new OpenSearchProjectionIndexer(basicAuthConnectionSettings, loggerFactory);
        _retryPipeline = BuildResiliencePipeline(resilienceSettings ?? ResilienceSettings.Default, _logger);
    }
    ...
    private static ResiliencePipeline BuildResiliencePipeline(ResilienceSettings settings, ILogger logger)
    {
        var shouldHandle = new PredicateBuilder<object>()
            .Handle<OpenSearchClientException>(ex =>
        ...
        return ResiliencePipelineFactory.Create(settings, shouldHandle, logger, "OpenSearch");
    }
    ...
    /// <summary>
    /// Bulk upsert using OpenSearch _bulk API — single HTTP request for all documents.
    /// </summary>
        ...
        await ExecuteWithRetriesAsync(async (ct) =>
        {
            ...
            try
            {
                ...
                if (response.Errors)
                {
                    ...
                    throw new Exception(
                        $"OpenSearch _bulk operation had errors on \"{indexDescriptor.IndexName}\". " +
                        $"First error: {firstError?.Error?.Type} - {firstError?.Error?.Reason}"
                    );
                }
            }
            catch (Exception ex) when (ex is not Exception { Message: var m } || !m.StartsWith("OpenSearch _bulk"))
            {
    ...
    private object? DeserializeDictionaryItem(object? item, ProjectionDocumentPropertySchema propertySchema)
    {
        ...
        // OpenSearch client returns int as long for some reason
        ...
        return await ExecuteWithRetriesAsync(async (ct) =>
        {
            try
            {
                ...
                var filters = query.Filters != null && query.Filters.Any()
                    ? OpenSearchFilterFactory.ConstructFilters(query.Filters, _projectionDocumentSchema)
    ...
    private QueryContainer ConstructSearchQuery<T>(QueryContainerDescriptor<T> searchDescriptor, ProjectionQuery projectionQuery) where T : class
    {
        ...
        {
            var queries = OpenSearchQueryFactory.ConstructSearchQuery(_projectionDocumentSchema, projectionQuery.SearchText);
        ...
        // construct filters
        var filters = OpenSearchFilterFactory.ConstructFilters(projectionQuery.Filters);
